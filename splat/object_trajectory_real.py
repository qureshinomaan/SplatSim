...
cubeid = p.loadURDF(models_lib[object_name], [0.5, 0.15, 0.0],  p.getQuaternionFromEuler([0,0,0]), flags=flags, useFixedBase=False)
...
aabb = p.getAABB(cubeid)
print('aabb:', aabb)
#move object up by z of the aabb
# p.resetBasePositionAndOrientation(cubeid, [0.0, 0.5, - aabb[0][2]], [0, 0, 0, 1])
#dynamics of the object
p.changeDynamics(cubeid, -1, mass=2, lateralFriction=0.9, spinningFriction=0.5)
# exit()
...
for i in range(240):
    ...
    object_pos, object_ori = p.getBasePositionAndOrientation(cubeid)
