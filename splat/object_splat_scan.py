...
def load_textures(texture_folder):
    textures = [os.path.join(texture_folder, f) for f in os.listdir(texture_folder) if f.endswith('.png')]
    return [p.loadTexture(tex) for tex in textures]

def create_textured_walls(textures):
    wall_size = [2, 2, 0.1]  # Size of each wall (length, height, thickness)
    wall_positions = [
        [0, 0, 2],  # Front wall
        [0, 0, -2],  # Back wall
        [2, 0, 0],  # Right wall
...
if __name__ == "__main__":
    ...
    pandaUid = p.loadURDF(args.urdf_path, useFixedBase=True)

    output_folder = "random_views_" + object_name
