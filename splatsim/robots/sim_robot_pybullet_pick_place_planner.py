...
class PybulletRobotServer:
    def __init__(
        ...
        self.object_name_list = ['plastic_apple', 'plastic_banana',  'plastic_strawberry']
        self.splat_object_name_list = ['plastic_apple', 'plastic_banana',  'plastic_strawberry']
        self.randomize_object_positions = [True, True, False]
        self.randomize_object_rotations = [False, True, True]
        self.rotation_values = [[0, 0], [0, 0], [-np.pi/6, np.pi/6]]
        self.use_fixed_base = [False, False, True]
        global_scaling_list = [1, 1, 0.8]
        ...
        self.grasp_poses = [self.apple_grasp_pose, self.banana_grasp_pose_1]
    ...
    def serve(self) -> None:
        ...
        while True:
            ...
            euler_z =  np.pi/2 + random.uniform(-np.pi/8, np.pi/8)
            ...
            while collison_between_objects:
                ...
                #check the distance of objects from the drop location
                obj_location, obj_quat = self.pybullet_client.getBasePositionAndOrientation(self.urdf_object_list[0])
                obj_location_2, obj_quat_2 = self.pybullet_client.getBasePositionAndOrientation(self.urdf_object_list[1])

                distance1 = np.linalg.norm(np.array(obj_location)[:2] - np.array(self.drop_ee_pos)[:2])
                distance2 = np.linalg.norm(np.array(obj_location_2)[:2] - np.array(self.drop_ee_pos)[:2])
            ...
            if random.uniform(0, 1) > 0.3:
                random_ee_pos = np.array([random.uniform(0.2, 0.5), random.uniform(-0.4, 0.3), random.uniform(0.25, 0.45)])
            else:
                #get object position
                choose_object = random.choice([0, 1])
                object_pos, object_quat = self.pybullet_client.getBasePositionAndOrientation(self.urdf_object_list[choose_object])
            ...
            #get the euler angles from the quaternion
            random_ee_euler = (-np.pi/2, 0, random.uniform(-np.pi/2 -np.pi/4, -np.pi/2 + np.pi/4))
            #get quaternion from euler angles
            random_ee_quat = self.pybullet_client.getQuaternionFromEuler(random_ee_euler)
            ...
            # skip_recording_first = random.choice([0, 1])
            ...
            if skip_recording_first:
                ...
                object_pos_2, object_quat_2 = self.pybullet_client.getBasePositionAndOrientation(self.urdf_object_list[1])


                #find the distance of the objects from the drop location
                distance1 = np.linalg.norm(np.array(object_pos)[:2] - np.array(self.drop_ee_pos)[:2])
                distance2 = np.linalg.norm(np.array(object_pos_2)[:2] - np.array(self.drop_ee_pos)[:2])
            ...
            for object in object_order:
                ...
                ik_joint_positions = []
                path = plan_joint_motion(self.dummy_robot, ik_joints, joint_positions, obstacles=[self.plane, self.urdf_object_list[0], self.urdf_object_list[1], self.urdf_object_list[-1]], self_collisions=False)
                ...
                if path is not None :
                    ...
                    break
                #append pre_grasp2grasp_path[0] tp start of pre_grasp2grasp_path
                pre_grasp2grasp_path = [pre_grasp2grasp_path[0] for _ in range(3)] + pre_grasp2grasp_path
            ...
            for k in range(2):
                ...
                #go to first object
                self.follow_trajectory(all_paths[4*k + 0], 0, gripper_velocity=0.2, skip_recording_first=skip_recording_first, error_threshold=0.001)
                ...
                # go to pregrasp
                self.follow_trajectory(all_paths[4*k + 1], 1, skip_recording_first=skip_recording_first)

                # go to intermediate
                self.follow_trajectory(all_paths[4*k + 2], 1, skip_recording_first=skip_recording_first)
            ...
            path = [initial_joint_state for _ in range(4)]
            self.follow_trajectory(path, 0)
    ...
    def setup_gripper(self):
        ...
        for joint_id, multiplier in self.mimic_child_multiplier.items():
            ...
            self.pybullet_client.changeConstraint(c, gearRatio=-multiplier, maxForce=10, erp=1)  # Note: the mysterious `erp` is of EXTREME importance
    ...
    def pre_grasp_to_grasp(self, transformation_matrix):
        ...
        if angle > 0.1 or angle < -0.1:
            print('angle is greater than 0.2')
            return None, None
        
        print('angle with z axis', angle)
    ...
    def follow_trajectory(self, path, gripper_pos, use_current_iters=True, gripper_velocity=2, skip_recording_first=0, error_threshold=1e-2):
        k = 0
        loop_iters = 0
        current_iters = 0 
        while k < len(path):
