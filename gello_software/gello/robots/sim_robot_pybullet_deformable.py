...
class PybulletRobotServer:
    def __init__(
        ...
        self.pybullet_client.resetSimulation(p.RESET_USE_DEFORMABLE_WORLD)
        ...
        clothId = self.pybullet_client.loadSoftBody("cloth_z_up.obj", basePosition = [0,0,0], 
                         scale = 0.1, 
                         mass = 5., 
                         useNeoHookean = 0, 
                         useBendingSprings=0,
        ...
        self.urdf_object_list.append(clothId)

        self.pybullet_client.changeVisualShape(clothId, -1, flags=self.pybullet_client.VISUAL_SHAPE_DOUBLE_SIDED)
        self.pybullet_client.setPhysicsEngineParameter(sparseSdfVoxelSize=0.25)

        #load the mesh of the object
        mesh = trimesh.load(pybullet_data.getDataPath() + "/cloth_z_up.obj")

        self.faces = np.asarray(mesh.faces)

        #record the vertices of each face initially in the mesh

        mesh_data = self.pybullet_client.getMeshData(clothId, flags=self.pybullet_client.MESH_DATA_SIMULATION_MESH)
        mesh_data = np.asarray(mesh_data[1])
        self.initial_face_vertices = []
        for i in range(len(self.faces)):
            self.initial_face_vertices.append(mesh_data[self.faces[i]])

        print('self.initial_face_vertices', self.initial_face_vertices)
        ...
        x = random.uniform(0.4, 0.6)
        y = random.uniform(-0.3, 0.3)
        #the cloth should face the z axis
        quat = self.pybullet_client.getQuaternionFromEuler([0, 0, np.pi/2])
        self.pybullet_client.resetBasePositionAndOrientation(self.urdf_object_list[0], [x, y, 0.01], quat)
        self.pybullet_client.createSoftBodyAnchor(clothId, 24, -1, -1)
        # self.pybullet_client.createSoftBodyAnchor(clothId, 20, -1, -1)
    ...
    def get_observations(self) -> Dict[str, np.ndarray]:
        ...
        # Get current vertices of the cloth directly from PyBullet
        mesh_data = self.pybullet_client.getMeshData(self.urdf_object_list[0], flags=self.pybullet_client.MESH_DATA_SIMULATION_MESH)
        vertices = np.array(mesh_data[1])  # mesh_data[1] contains the vertices

        # Compute transformation for each face
        face_transformations = []
        for i in range(len(self.faces)):
            # Get current vertices for this face using PyBullet mesh data
            current_face_vertices = vertices[self.faces[i]]
            initial_face_vertices = np.array(self.initial_face_vertices[i])
            
            # Compute centroids
            current_centroid = np.mean(current_face_vertices, axis=0)
            initial_centroid = np.mean(initial_face_vertices, axis=0)
            
            # Center the vertices
            current_centered = current_face_vertices - current_centroid
            initial_centered = initial_face_vertices - initial_centroid
            
            # Compute optimal rotation using SVD
            H = initial_centered.T @ current_centered
            ...
            # Compute translation
            t = current_centroid - R @ initial_centroid
            ...
            face_transformations.append(transformation)
        
        # Store each face transformation individually
        for i, transformation in enumerate(face_transformations):
            observations[f"cloth_face_{i}_transform"] = transformation
